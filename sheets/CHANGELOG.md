...
### Diubah (post-1.0.0)
- **Semua dialog konfirmasi diganti pakai SweetAlert2** (dimuat dari CDN saat dibutuhkan, konsisten dengan pola Prism.js/ExcelJS — bukan dependency npm) — menggantikan `window.confirm()` bawaan browser yang tampilannya kaku & tidak konsisten antar OS/browser. Tampilannya otomatis mengikuti tema dark/light aplikasi. Kalau CDN gagal dimuat (mis. offline), otomatis fallback ke `confirm()` bawaan browser supaya fitur tetap berfungsi.
