...
UI.routes.settings = async function (view) {
  ...
  view.innerHTML = UI.topbar('Settings', 'Konfigurasi AI & tampilan — tersimpan di Google Sheets, berlaku untuk semua perangkat') +
    ...
    '    <option value="gemini"' + (s.provider === 'gemini' ? ' selected' : '') + '>Google Gemini (generateContent API)</option></select>' +
    ...
    '    <input class="input mono" id="sKey" type="password" placeholder="' + (s.hasApiKey ? 'Kosongkan jika tidak ingin mengganti' : 'AIzaSy…') + '">' +
    '    <p class="small muted" style="margin:6px 0 0">Dapatkan API Key gratis di <span class="mono">aistudio.google.com/apikey</span></p></div>' +
